# Ensure we run from the script's directory (critical when "Run with PowerShell" from Explorer)
...
# Run pm2 command (use absolute path for --env-file so PM2 child resolves it correctly)
$envFile = Join-Path $PSScriptRoot ".env"
npx pm2 start (Join-Path $PSScriptRoot "index.js") --name jkn-fp-bot --node-args="--env-file=$envFile"
...
$batFileContent = "@echo off`nnpx pm2 resurrect`nexit"
